<?php
require_once('tcpdf/tcpdf.php'); // Make sure to adjust the path to the TCPDF library if needed
...
$pdf->SetY(20); // Adjust the Y coordinate to move the email address closer
$pdf->Cell(0, 0, 'Email: support@armarra.com', 0, 1, 'L');

// Add title
$pdf->SetFont('helvetica', 'B', 15);
$pdf->SetY(25); // Adjust the Y coordinate to move the title closer
$pdf->Cell(0, 0, 'Sales Report', 0, 1, 'C');

// Set font
$pdf->SetFont('helvetica', 'B', 11);
// Create a table for customer data
$html = '<table border="1"><tr><th>Customer Name</th><th>Customer Email</th><th>TrxID</th><th>Customer Last Order Date</th><th>Customer Total Spent</th><th>Payment Status</th></tr>';
$rowCount = 0; // Keep track of rows

foreach ($customerNames as $i => $customerName) {
    $html .= '<tr>';
    $html .= '<td>' . $customerName . '</td>';
    $html .= '<td>' . $customerEmails[$i] . '</td>';
    $html .= '<td>' . $additionalData[$i]['Txid'] . '</td>';
    $html .= '<td>' . $additionalData[$i]['Date'] . '</td>';
    $html .= '<td>' . $additionalData[$i]['Usd'] . '</td>';
    $html .= '<td>' . $additionalData[$i]['paymentStatus'] . '</td>';
    $html .= '</tr>';
    
    $rowCount++;

    // Add a page break after every 6 rows
    if ($rowCount === 7 && $i !== count($customerNames) - 1) {
        $html .= '</table>'; // Close the current table
        $pdf->writeHTML($html, true, false, true, false, ''); // Output the table
...
// Output the last part of the table
if ($rowCount > 0) {
    $html .= '</table>';
...
$pdf->Output('sales_report.pdf', 'I'); // 'I' sends the PDF to the browser, 'F' saves it to a file
?>
